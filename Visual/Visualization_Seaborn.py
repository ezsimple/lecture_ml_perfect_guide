...
# distplot은 더이상 사용하지 마세요
# (주의) y축이 density 가 나옵니다.
sns.distplot(titanic_df['Age'], bins=50)
...
# distplot 대신 histplot 을 사용해야 합니다.
# y축은 count로 나옵니다.
# kde : kernel density estimation -> 곡선 그래프가 추가 됩니다.
sns.histplot(titanic_df['Age'], kde=True, bins=30)
#plt.show()


# %%
# (주의) data로 전체 df 를 전달해도 됩니다.
# plt.figure(figsize=(12, 6))
# rug 없음, height, aspect 가 없습니다.
...
# Pandas DataFrame의 컬럼명을 자동으로 인식해서 xlabel값을 할당. ylabel 값은 histogram일때 Count 할당.
# rug : 양탄자
# (주의) height, width 가 아닌 height & aspect 입니다.
# (주의) displot은 plt.figure로 사이즈를 지정할 수 없습니다.
# (주의) displot 입니다. (distplot이 아닙니다.) ==> histplot 과 같은 히스토그램이 출력됩니다.
# histplot 대신 기본으로 displot 을 사용하는 게 나을 것 같습니다.
# histplot과 거의 같지만, rug가 있습니다. histplot에는 rug가 없습니다.
...
# %%
# distplot은 plt.figure로 사이즈를 지정할 수 있습니다.
# (주의) displot은 plt.figure로 사이즈를 지정할 수 없습니다.
plt.figure(figsize=(10, 6))
...
### (주의) 수직 barplot에 y축을 문자값으로 설정하면 자동으로 수평 barplot으로 변환
### 임의로 변경해 버리므로, 원하는 그래프가 아닙니다.
sns.barplot(x='Pclass', y='Sex', data=titanic_df)


# %%
# ci = confidence interval을 없애고, color를 통일.
# ci : 평균 +- t.statistics(샘플값)
...
data = titanic_df.groupby('Pclass')
data.describe()
...
# (중요) hue 인자가 중요합니다.
# group by Pclass
sns.barplot(x='Pclass', y='Age', hue='Sex', ci=None, data=titanic_df)
...
# (주의) orient가 h이면, x가 숫자형 값이고, y값이 카테고리 값 유형 이어야 합니다.
